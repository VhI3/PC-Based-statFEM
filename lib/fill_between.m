...
function h = fill_between(x, y1, y2, where, varargin)
    ...
    for n = 1:max(cat)
        % ---------------------------------
        h(n) = fill_patch(x, y1, y2, cat == n, EdgeColor, FaceColor, tranc);
        % ---------------------------------
    end

    %% Now plot the full x,y1 and x,y2 lines
    hold on
    %y1handle = plot(x,y1,'w-','HandleVisibility','off');
    %y2handle = plot(x,y2,'w-','HandleVisibility','off');
...
function h = fill_patch(x, y1, y2, where, EdgeColor, FaceColor, tranc)
